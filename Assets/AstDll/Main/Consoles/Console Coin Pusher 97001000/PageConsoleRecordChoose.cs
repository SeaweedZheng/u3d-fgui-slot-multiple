        ...
        void OnClickConfirm()
        {
            if (menuMap.ContainsKey(curIndexMenuItem))
            {
                ...
                switch (menuMap[curIndexMenuItem])
                {
                    case "businessRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97001000PageConsoleBusinessRecord);
                        }
                        return;
                    case "coinRecord":
                        {
                    ...
                    case "eventRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97001000PageConsoleEventRecord);
                        }
                        return;
                    case "erroRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97001000PageConsoleErrorRecord);
                        }
                        return;
                    case "exit":
                        {
