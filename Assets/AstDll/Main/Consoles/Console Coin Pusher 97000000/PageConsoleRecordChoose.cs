...
namespace ConsoleCoinPusher97000000
{
    public class PageConsoleRecordChoose : MachinePageBase
    {
        ...
        {
            PageTitleManager.Instance.AddPageNode("Record");
        ...
        void OnClickConfirm()
        {
            if (menuMap.ContainsKey(curIndexMenuItem))
            {
                ...
                switch (menuMap[curIndexMenuItem])
                {
                    case "businessRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97000000PageConsoleBusinessRecord);
                        }
                        return;
                    case "coinRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97000000PageConsoleCheckHardware);
                        }
                        return;
                    case "gameRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97000000PageConsoleTestCoinPush);
                        }
                        return;
                    case "jpRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97000000PageConsoleSettings);
                        }
                        return;
                    case "eventRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97000000PageConsoleEventRecord);
                        }
                        return;
                    case "erroRecord":
                        {
                            PageManager.Instance.OpenPage(PageName.ConsolePusher97000000PageConsoleErrorRecord);
                        }
                        return;
                    case "exit":
                        {
