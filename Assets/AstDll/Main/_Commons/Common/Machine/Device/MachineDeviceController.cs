...
public class MachineDeviceController  : MonoSingleton<MachineDeviceController>  // 
{
    ...
    public readonly Dictionary<ulong, MachineButtonKey> keyMap = new Dictionary<ulong, MachineButtonKey>()
    {
        ...
        //游戏按钮：
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_SPIN ,MachineButtonKey.BtnSpin},  // 开始玩 或 确认
        //##?? { { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.SWITCH_SWITCH, MachineButtonKey.BtnExit},
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_HELP,MachineButtonKey.BtnPayTable},

        //管理按钮：
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_PAYOUT ,MachineButtonKey.BtnTicketOut},  // 下一页
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_SCORE_UP ,MachineButtonKey.BtnCreditUp},
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_SCORE_DOWN ,MachineButtonKey.BtnCreditDown},
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_SET ,MachineButtonKey.BtnConsole},  // 进入 或 退出 后台
       //##?? { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.SWITCH_DOOR_SWITCH ,MachineButtonKey.BtnDoor}
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_ACCOUNT ,MachineButtonKey.BtnUp},
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_AUTO ,MachineButtonKey.BtnDown},
        { (ulong)SBOX_IDEA_COIN_PUSH_KEYVALUE.KV_SWITCH ,MachineButtonKey.BtnSwitch},  // 雨刷  或 上一页
    };
...
#region 按钮检查
    void Update()
    {
        ...
        if (SBoxIdea.GetCoinPushKeyState()!=0)
            GetPressedButtons(SBoxIdea.GetCoinPushKeyState());
    ...
    private Dictionary<SBOX_IDEA_COIN_PUSH_KEYVALUE, float> btnStartTimeInfos 
        = new Dictionary<SBOX_IDEA_COIN_PUSH_KEYVALUE, float>();
    
    public  void GetPressedButtons(int buttonValue)
    {
        // 遍历枚举的所有值
...
#region  按钮长按逻辑
    ...
    bool IsSysPriority(MachineButtonKey value)
    {
        if (value == MachineButtonKey.BtnConsole 
            //&& PageManager.Instance.IndexOf(PageName.ConsolePusher97000000PageConsoleMain) == -1
            && !MainModel.Instance.isOpenConsolePage
            )
        {
            return true;
        }
