...
public class MachineDeviceCommonBiz : MonoSingleton<MachineDeviceCommonBiz>
{
    ...
    public void OpenConsole()
    {
        ...
        // 受到保护时，不允许打开管理后台
        if (GlobalModel.isProtectApplication) return;
    ...
    {
        if (PageManager.Instance.IndexOf(PageName.ConsolePusher97000000PageConsoleMain) != -1)
            yield break;
        ...
        yield return new WaitUntil(() => MainModel.Instance.isSpin == false);


        MainModel.Instance.contentMD.isRequestToStop = false;

        if (PageManager.Instance.IndexOf(PageName.ConsolePusher97000000PageConsoleMain) == -1)
            PageManager.Instance.OpenPage(PageName.ConsolePusher97000000PageConsoleMain);
