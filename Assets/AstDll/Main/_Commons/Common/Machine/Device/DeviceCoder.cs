...
public class DeviceCoder : MonoSingleton<DeviceCoder>
{
    ...
    private void CheckSBoxNeedActivated()
    {
        ...
        MachineDataManager02.Instance.RequestIsCodingActive((res) =>
        {
            ...
            {
                //控台没有打开
                //if (PageManager.Instance.IndexOf(PageName.ConsolePageConsoleMain) == -1   && PageManager.Instance.IndexOf(PageName.ConsolePusher97000000PageConsoleMain) == -1)
    ...
    private async void OnResponseShowCoder(SBoxCoderData data , PageName pageName)   // string pageName)
    {
        ...
        {
            res = await PageManager.Instance.OpenPageAsync(PageName.ConsoleSlot98000000PopupConsoleCoder,  new EventData<Dictionary<string, object>>("", req));
        }else if (pageName == PageName.ConsolePusher97000000PageConsoleCoder)//(pageName == "ConsolePusher97000000PageConsoleCoder")
        {
            res = await PageManager.Instance.OpenPageAsync(PageName.ConsolePusher97000000PageConsoleCoder, new EventData<Dictionary<string, object>>("", req));
        }
        else if (pageName == PageName.ConsolePusher97001000PageConsoleCoder)//(pageName == "ConsolePusher97000000PageConsoleCoder")
        {
            res = await PageManager.Instance.OpenPageAsync(PageName.ConsolePusher97001000PageConsoleCoder, new EventData<Dictionary<string, object>>("", req));
        }*/
        if (pageName == PageName.ConsoleSlot98000000PopupConsoleCoder 
