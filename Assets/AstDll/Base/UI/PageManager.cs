...
public partial class PageManager : MonoBehaviour
{
    ...
    public void OpenPage(PageName pageName, InParamsBase data = null, Action<PageBase> onFinishCalllback = null)
    {
        ...
        // 检查路径是否配置
        object[] confg = null;
        if (!UIRouter.Instance.pathDict.TryGetValue(pageName, out confg))
        {
        ...
        if (windowCtrl == null)
        {
            Assembly hotfixAssembly = Assembly.Load("Main");
        ...
        Action funcAfterLoadBundle = () =>
        {
            // 创建前
            MethodInfo beforeCreatMethod = windowCtrl.GetMethod(
                name: PageBase.ON_BEFORE_CREAT,
                bindingAttr: BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, // 兼容公私方法
                binder: null,
                //types: new[] { typeof(Action<object>) }, // 匹配参数：Action<object> onFinishCallback
                types: new[] { typeof(Action) },
                modifiers: null
            );

            if (beforeCreatMethod != null)
            {
                beforeCreatMethod.Invoke(
                    obj: null, // 静态方法无实例，传 null
                    parameters: new object[] { funcStartCreat } // 回调函数作为参数传入
                );
            }
            else
            {
