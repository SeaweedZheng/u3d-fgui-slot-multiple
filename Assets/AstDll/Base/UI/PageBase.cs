...
public class PageBase : Window
{
    ...
    public const string ON_BEFORE_CREAT = "OnBeforeCreat";  //可选
    ...
    protected virtual void OnBeforeCreat(Action onFinishCallback)
    {
    ...
    */
    ...
    private  async Task WaitUntil(Func<bool> condition)
    {
        while (!condition())
        {
            ...
            {
                DebugUtils.LogWarning($"【PageBase】编辑器-停止运行，WaitUntil等待跳出。{pageName}");
                return;
            }
        }
        // 打印等待结束的日志
        //DebugUtils.LogWarning($"【PageBase】WaitUntil等待结束，条件已满足！{pageName}");
    }
