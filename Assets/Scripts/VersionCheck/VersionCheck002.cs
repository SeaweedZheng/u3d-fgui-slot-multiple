...
public class VersionCheck002 : MonoBehaviour
{
    ...
    string GetWebAstBackupHash(string nodeName) => versionFileRemoteNode["asset_backup"][nodeName]["hash"].ToObject<string>();

    /// <summary> 获取本地dll文件的hash  </summary>
    string GetLocalAstBackupHash(string nodeName) => GlobalModel.version["asset_backup"][nodeName]["hash"].ToObject<string>();
    ...
    /// <returns></returns>
    string GetWebHotfixDllHash(string dllName) => versionFileRemoteNode["asset_dll"][dllName]["hash"].ToObject<string>();
    ...
    /// <param name="dllName"></param>
    /// <returns></returns>
    string GetLocalHotfixDllHash(string dllName) => GlobalModel.version["asset_dll"][dllName]["hash"].ToObject<string>();
    ...
    public IEnumerator DoHotfix(UnityAction callback)
    {
        ...
        {
            Debug.LogError($"last hotfix throw err: {PlayerPrefs.GetString(HotfixState.HOTFIX_THROW_ERR)}");
        }
        ...
        else
        {
            ...
            string localVersionVER = GlobalModel.version["hotfix_version"].Value<string>();
            ...
            if (localVersions[0] == serverVersions[0])
            {
                ...
                if (localVersions[1] != serverVersions[1] || localVersions[2] != serverVersions[2])
                {
                    ...
                    if (PlayerPrefs.GetString(HotfixState.HOTFIX_STATE) != HotfixState.HotfixDownloadFail)
                    ...
                    if (PlayerPrefs.GetString(HotfixState.HOTFIX_STATE) != HotfixState.HotfixDownloadFail)
                        yield return StartDownloadAllAstBackup(); //下载 备份资源
    ...
    private IEnumerator DeleteUnuseAssetDll()
    {
        ...
        JObject assetDlls = GlobalModel.version["asset_dll"] as JObject;
    ...
    /*
    public IEnumerable DeleteMetaFile()
    {
        int i = 0;
        List<string> metaFiles = GetTargetFilePath(PathHelper.hotfixDirLOCPTH, ".meta");
        foreach (string s in metaFiles)
        {
            i++;
            if (File.Exists(s))
            {
                PageLaunch.Instance.Next(LoadingProgress.DELETE_UNUSE_ASSET_DLL, $"delete meta file {i}/{metaFiles.Count}: {s}  ");

                Debug.Log($"delete unuse dll {i}/{metaFiles.Count}: {s}");
    ...
    #region 远端下载备份资源
    private IEnumerator DownloadRemoteAstBackupOnce(string nodeName)
    {
        if (PlayerPrefs.GetString(HotfixState.HOTFIX_STATE) == HotfixState.HotfixDownloadFail)
            yield break;

        string hashRemote = GetWebAstBackupHash(nodeName);
        string astBackupDownloadUrl = PathHelper.GetAssetBackupWEBURL(nodeName);

        // cdn 加载
        UnityWebRequest req = UnityWebRequest.Get(astBackupDownloadUrl); //ApplicationSettings.Instance.hotfixUrl
        ...
        // 非cdn加载
        UnityWebRequest req01 = UnityWebRequest.Get(astBackupDownloadUrl + $"?t={DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}");
        ...
        HotfixState.hotfixThrowErrMsg = $"down asset backup '{astBackupDownloadUrl}' is fail";
        throw new Exception(HotfixState.hotfixThrowErrMsg);
    }




    private IEnumerator StartDownloadAllAstBackup()
    {
        Debug.LogWarning($"{PREFIX_TIP} start download asset backup");

        JObject localBackupNode = GlobalModel.version["asset_backup"] as JObject;
        ...
        foreach (KeyValuePair<string, JToken> item in serverBackupNode)
        {
                ...
                || (localBackupNode[name]["hash"].ToObject<string>() != serverBackupNode[name]["hash"].ToObject<string>()))
            {
                ...
                yield return DownloadRemoteAstBackupOnce(name);
            }
            i++;
        }
    ...
    private IEnumerator DownLoadRemoteHotfixDllOnce(string dallame)
    {
        ...
        hotfixDllDownloadUrl = hotfixDllDownloadUrl.Replace("//AstDll", "/AstDll");
        ...
        HotfixState.hotfixThrowErrMsg = $"down dll '{hotfixDllDownloadUrl}' is fail";
        throw new Exception(HotfixState.hotfixThrowErrMsg);
    }
    ...
    private IEnumerator StartDownloadAllHotfixDll()
    {
        ...
        JObject localHotfixNode = GlobalModel.version["asset_dll"] as JObject;
        ...
        {   
            using (UnityWebRequest webReq = UnityWebRequest.Get($"{path}?t={DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}"))
            {
                ...
                else
                {
                    ...
                    HotfixState.hotfixThrowErrMsg = $"download bundle: {iter} ; path: '{path}' ; error: {webReq.error}";
                    Debug.LogError(HotfixState.hotfixThrowErrMsg);
                    yield return null;
                }
            }
